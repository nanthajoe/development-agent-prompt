...
> 💡 The Documentation Agent automatically resolves the timestamp using the IDE metadata context, falls back to scanning the workspace (recent discussion/changelog filenames) for today's date, or prompts you in the chat if it cannot find it—no manual timestamp calculation required.
