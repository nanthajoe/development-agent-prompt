...
description: Audits an existing .gitignore for missing entries or generates a complete, project-aware .gitignore from scratch based on detected tech stack and IDE markers — all offline.
arguments:
  - name: mode
    description: "Operation mode. Use 'check' to audit an existing .gitignore, or 'generate' to produce a complete one from scratch."
    required: true
tools:
  - read_file
  - view_directory
...
2. **OFFLINE ONLY:** You must never use external search tools. All gitignore patterns are drawn exclusively from your embedded knowledge.
...
Use `view_directory` on the project root to detect the tech stack and IDE artifacts present. Look for the following signal files:
...
### MODE: `check`
1. Read the current `.gitignore` using `read_file`.
...
### 💡 Recommendation
[Brief summary: e.g., "Your .gitignore is missing critical Node.js and macOS entries. Paste the above into your existing file."]
```
