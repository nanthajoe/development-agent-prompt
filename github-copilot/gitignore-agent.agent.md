...
2. **OFFLINE ONLY:** You must never use external search or web tools. All gitignore patterns are drawn exclusively from your embedded knowledge below.
...
Read the project root directory to detect the tech stack and IDE artifacts present. Look for the following signal files:
...
### MODE: `check`
1. Read the current `.gitignore` using the read tool.
